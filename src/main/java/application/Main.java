...
public class Main extends Application {
	...
	private Image p;
	private Label nombre_cancion, listado_canciones;
	private ListView<File> Lista_Vi;
	private ObservableList<File> list_Obser;
	private String nombre_cancion_escogida, pasamos_nombre_cancion, nombre_borrar_cancion, dire;
	...
	// private final String Ruta = "/Volumes/NO
	// NAME/JSP/FX_AUDIO_EJERCICIO/src/audio/";
	private File apunte, prueba2;
	private Modelo canciones_escogidas;
	private Control uno;
	//private Control reproduccir;
...
// ************************************************************************************************************************************************************************
			/*
			 * A) CREAMOS UN OBJETO DE ARCHIVO B) UNA LISTA DE OBJETOS DE ARCHIVOS C)
			 * INSTANCIAMOS EL OBEJETO QUE SERÁ EL ENCARGADO DE PODER DESARROLLAR TODO EL
			 * PROGRAMA. ESE OBJETO ES EL QUE CONTIENE TODO LA LOGICA DEL PROGRAMA.
			 */
			...
			uno = new Control(canciones_escogidas.nextSong());
			
			/*dire = System.getProperty("user.dir") + "/";
			System.out.println(dire);*/
			if(!lista_Canciones_Directorio.isEmpty()) {
				File f = lista_Canciones_Directorio.get(0);
				dire =	f.getParent() + "/";
...
//							canciones_escogidas);
					uno = new Control(lista_Canciones_Directorio, Lista_Vi , listado_canciones);
...
// ************************************************************************************************************************************************************************
			...
			/*
			 * En el siguiente bucle implementamos a la List View todas las canciones del
			 * ArrayList inicializado en la clase Modelo.
			 * 
			 * Nota: En la variables almacenamos los nombres de las canciones - sin la
			 * dirección absoluta -. Estás serán las que verán los usuarios en sus
			 * dispositivos. Posteriormente tendremos que concatenar la ruta completa.
			 */
				...
				public void changed(ObservableValue observable, Object oldValue, Object newValue) {
					if (!reproduccion_auto.isSelected()) {
						...
						for (File i : list_Obser) {
							try {
								// --- Se recorre la ObservableList donde está añadidas todas las canciones del
								// ListView.
								...
								// --- Concatenamos la ruta completa para inicializar el objeto Control que es
								// donde está los métodos que hacen reproduccir la música.
								/*
								 * System.out.println(dire + "/src/main/java/audio/" + nombre_cancion_escogida);
								 * apunte = new File(dire + "/src/main/java/audio/" + nombre_cancion_escogida);
								 */
								// System.out.print(i.getAbsoluteFile());
								System.out.println(dire + nombre_cancion_escogida);
								apunte = new File(dire + nombre_cancion_escogida);

								uno = new Control(apunte);
								// --- Esta línea de código es fundamental para conocer la posición de la
								// canción. Usada en los métodos de la clase Modelos - que es a la que per
								...
								System.out.println(
										"Error en la clase principal - main - en la interación del list_Obser");
							}
						}
...
// ************************************************************************************************************************************************************************			
			...
			menuItem_añadir_cancion.setOnAction(e -> {
				/*
				 * -- ACUDO AL MÉTODO DE LA CLASE MODELO QUE REALIZA LAS SIGUIENTES ACCIONES: A)
				 * ABRE EL DIRECTORIO LOCAL BUSCANDO NUEVAS CANCIONES. B) MEDIANTE UN FOR EACH
				 * SE AÑADEN LAS CANCIONES CRONOLÓGICAMENTE. C) SE ELIMINAN LAS DUPLICIDADES. D)
				 * CON UN SEGUNDO BUCLE MOSTRAMOS LA LISTA FINAL DE LAS CANCIONES
				 */
				canciones_escogidas.anadir_cancion(primaryStage);
			...
			menuItemInfo.setOnAction(e -> {
				...
				nueva.mostrar(
						"    Author: Daniel GM" + "\n" + "    Version: Player 1.0" + "\n" + "    Company: WillsCompany",
						"Player Information", 550, 800);
			});
...
// ************************************************************************************************************************************************************************
			...
			VBox contenedor_vertical = new VBox(20, HORIZONTAL_MENU, HORIZONTAL_LOGO, HORIZONTAL_VOLUMEN,
					HORIZONTAL_BOTONES, HORIZONTAL_TITULO_CANCION, VERTICAL_LISTADO_CANCIONES, REPRODUCCION_AUTOMATICA);
	...
	public void metodo_atrasar() {
		...
		uno = new Control(canciones_escogidas.previusSong());
		Lista_Vi.selectionModelProperty().get().select(canciones_escogidas.getId());
		uno.play();
